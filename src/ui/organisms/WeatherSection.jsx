  ...
  return (
    <section className={sc.weather_card} style={{ backgroundColor: cardColor }}>
      <div className={sc.desc}>
        ...
        <p className={sc.coord}>{coord.lat.toFixed(3)}, {coord.lon.toFixed(3)}</p>
      </div>
      <img
        className={sc.fav}
      ...
      <div className={sc.about}>
        ...
        <div className={sc.temp_container}>
          <p className={sc.temp}>{Math.round(main.temp)} <span>°C</span></p>
          <p className={sc.weather_type}>{weatherData.weather[0].description}</p>
        </div>
      </div>
    </section>
  );
